...
\end{figure}
	...
	Здесь \textcolor{purple}{фиолетовым} цветом отмечена восстановленная кривая, а \textcolor{green}{зеленым}~-- реальный график.
	\begin{figure}[H]
		\centering
	...
	Также сгенерируем точки относительно прямой теперь с такими параметрами:
	\begin{align*}
		\texttt{dots\_count} &= ???~\text{(количество генерируемых точек)} \\
		\texttt{variance} &= ???~\text{(вариативность случайности)}
	\end{align*}
	На выходе получаем такой график, где \textcolor{gray}{серым} отмечены сгенерированные данные для алгоритма, а \textcolor{green}{зеленым}~-- реальный график.
	\begin{figure}[H]
		\centering
	...
	Зададим ему следующие параметры на подбор функции по сгенерированным данным:
	\begin{align*}
		\texttt{batch\_size} &= ???~\text{(количество точек или, по простому, число $n$)} \\
		\texttt{max\_epochs} &= ???~\text{(максимальное число шагов до схождения)} \\
		\texttt{learning\_rate} &= ???~\text{(шаг в схождении к минимуму)}
	\end{align*}
	Получаем следующий график.
	Здесь \textcolor{purple}{фиолетовым} цветом отмечена восстановленная кривая, а \textcolor{green}{зеленым}~-- реальный график.
	\begin{figure}[H]
